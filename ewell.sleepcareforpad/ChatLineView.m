...
- (void)drawRect:(CGRect)rect {
    ...
    self.verticalSpace = 1.0/(self.xValueArr.count+1.0);
    //1绘制纵轴
    CGContextRef context = UIGraphicsGetCurrentContext();
    [[UIColor lightGrayColor]set];
    NSString *unitYStr = self.unitYStr;
    
    UIFont *font = [UIFont systemFontOfSize:16.0];//设置
 
    [unitYStr drawInRect:CGRectMake(distanceToLeft,8,250,10) withFont:font];
...
//        [self.yearArr[i] drawInRect:CGRectMake( x+10,8,50,10) withFont:font];
    ...
    for (int i = 0 ; i < self.yValueArr.count ; i++) {
        [self.yValueArr[i] drawInRect:CGRectMake(0,(self.yValueArr.count - i) * self.horizontalSpace,30,10) withFont:font];
    }
    
    //1.1绘制虚线
    [[UIColor colorWithRed:245/255.0 green: 245/255.0 blue: 245/255.0 alpha:1.0] set];
    for (int i = 0 ; i < self.yValueArr.count ; i++) {
        CGContextMoveToPoint(context, 15, (self.yValueArr.count - i) * self.horizontalSpace + 10);
    ...
    for (int i = 0 ; i < self.xValueArr.count ; i++) {
        ...
        if(self.xValueArr.count>14){
            ...
            [self.xValueArr[i] drawInRect:CGRectMake((i + 0.95) * (rect.size.width * self.verticalSpace),(self.yValueArr.count + 1) * self.horizontalSpace ,20 ,10) withFont:font];
        }
        else{
            [self.xValueArr[i] drawInRect:CGRectMake((i + 0.8) * (rect.size.width * self.verticalSpace),(self.yValueArr.count + 1) * self.horizontalSpace ,20 ,10) withFont:font];
        }
    }
    ...
    for (PointBtn * clickBtn in self.pointArr) {
        ...
        if (abs(touchpoint_x-btn_x)<8) {
            if (abs(touchpoint_y-btn_y)<8){
                ...
                UIColor *defaultColor = clickBtn.backgroundColor;
                UILabel *moneyLable = [[UILabel alloc]initWithFrame:CGRectMake(btn_x-5, btn_y-25,30, 20)];
                moneyLable.text = [NSString stringWithFormat:@"%@",clickBtn.valueStr];
